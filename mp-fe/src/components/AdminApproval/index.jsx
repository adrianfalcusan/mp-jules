...
const AdminApproval = () => {
  ...
  const fetchPendingContent = async () => {
    try {
      ...
      const response = await fetch(
        "http://localhost:8080/api/admin/approval/pending-approval",
        {
          headers: {
  ...
  const handleApprove = async (id, type) => {
    try {
      setProcessingId(id);
      const response = await fetch(
        `http://localhost:8080/api/admin/approval/approve-${type}/${id}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const handleReject = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/api/admin/approval/reject/${type}/${item.id}`,
        {
          method: "POST",
          headers: {
  ...
  const openPreview = async (id, type) => {
    try {
      const endpoint =
        type === "course"
          ? `http://localhost:8080/api/courses/${id}/content`
          : `http://localhost:8080/api/tutorials/${id}/content`;
      const res = await fetch(endpoint, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });
      const data = await res.json();
      const url = data?.videoLinks?.[0];
      if (!url) throw new Error("No preview URL available");
      window.open(url, "_blank", "noopener,noreferrer");
    } catch (e) {
      console.error("Preview error", e);
      setMessage("Failed to load preview: " + (e?.message || "Unknown error"));
    }
  };
          ...
          <Grid item xs={12} md={4}>
            <Stack spacing={2} alignItems="flex-end">
              ...
              <Button
                ...
                size="small"
                startIcon={<VideoIcon />}
                onClick={() => openPreview(item.id, type)}
                fullWidth
                sx={{ mb: 1 }}
              >
                Preview Video
        ...
        <Paper elevation={8} sx={{ borderRadius: 3, overflow: "hidden" }}>
          ...
          <Box sx={{ bgcolor: "primary.main" }}>
            ...
            >
              <Tab
                icon={<PendingIcon />}
                label={`Courses (${pendingContent.courses?.length || 0})`}
                iconPosition="start"
              />
              <Tab
                icon={<PendingIcon />}
                label={`Tutorials (${pendingContent.tutorials?.length || 0})`}
                iconPosition="start"
              />
            </Tabs>
          </Box>
